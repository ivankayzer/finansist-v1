    ...
    <section>
      <b-table
        ...
        <template slot-scope="props">
          <b-table-column field="id" label="ID" width="20" sortable numeric centered>
            {{ props.row.id }}
          </b-table-column>

          <b-table-column field="date" label="Дата" width="40" centered>
            {{ new Date(props.row.paid_at).toLocaleString('ru').split(',').shift() }}
          </b-table-column>
          ...
          <b-table-column field="amount" label="Сумма" width="50" sortable centered>
            <p :class="props.row.amount > 0 ? 'has-text-primary' : 'has-text-danger'">{{ props.row.amount }}</p>
          </b-table-column>
          ...
          <b-table-column field="is_formatted" label="Формат" width="20" centered>
                <span class="tag is-success" v-if="props.row.formatted_title && props.row.formatted_title.length">
                    Да
                </span>
