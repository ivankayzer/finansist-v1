...
class BudgetController {
  async all({ request, auth }) {
    ...
    for (let key in budgets) {
      ...
      let transactions = await user.transactions().expenses()
        .notIgnored()
        .betweenDates(dayjs(budget.start_date).format(), dayjs(budget.end_date).format())
        .fetch()
  ...
  aggregateTransactions(transactions) {
    return collect(transactions).groupBy('category_id').map(group => group.sum('amount'))
  }
}
